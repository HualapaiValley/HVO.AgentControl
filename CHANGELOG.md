...
- `home-docker` is deployed from promoted `main` `cae2ebc` at schema v13 for the #220 acceptance; `quick_check` is clean and FK errors are zero. The task-control capability flags in that currently deployed `cae2ebc` build are still `TaskControlOperationallyValidated=false` / `TaskControlValidatedScope=null`; this evidence change is a later, not-yet-promoted code line, so `/api/info` **will** report the flags below only after this evidence change is promoted. The task code itself at this evidence head (`9846991`) reports the new values. WorkerControl is enabled through the ignored Compose override, while the product/Compose default remains `true`.
- Live #219 hire approval/provisioning completed for `emp-933d24fc110222a5` at `Ready` with one worker container/four persistent volumes, live-model orientation comprehension, and no duplicates after restart/recovery. Live #257 rebuild r1→r2 completed `Applied` (epoch 600→604), preserving home/workspace/session hashes and one container/four volumes.
- #220 is now operationally validated in this evidence change (2026-09-21): live evidence was collected on the promoted `cae2ebc` / schema-v13 deployment, where the first local managed employee (`emp-933d24fc110222a5`) ran two bounded tasks end to end and each reached `Verified` through independent host verification on the controller-local Docker target, across a control restart, a cancellation/hold exercise, an orientation revision and an exact uncertainty recovery. This evidence code line (`9846991`, branch `docs/220-live-acceptance`) reports `TaskControlOperationallyValidated=true` and `TaskControlValidatedScope="first-local-managed-two-task-restart-verification"` once promoted; it is the next promoted evidence line, distinct from the currently deployed `cae2ebc` behavior above. There is no scheduler; cancellation is not rollback; there is no multi-agent routing, production repository/GitHub write or release publication.

### Added

- Completed #220 Phase 1 operational acceptance (2026-09-21) and updated capability truth. Promoted/deployed `main` evolved through `fa81286`, `5854601`, `579783f`, `3e496c4` and `cae2ebc` while live defects were reviewed and promoted; the current code line for these evidence docs is development `98b8f30`, schema v13, and the final production line will be a later promotion. The pre-deploy full snapshot `agentcontrol-v2-backup-pre-fa81286` verified 35 files / 9,399,203 bytes with exact schema-v12 DB hash `59f05e4fa5d94411258b74f645cdf37be5b9cca8e3fa6ba9303810ab8b61fcea`; the helper socket is `1002:1001` mode `0660`. The employee `emp-933d24fc110222a5`, binding `rtb-c9d9c8ddd79782ac`, worker `wrk-abd43903583bafdc827260fc` and native session `ses_f3faf2abeffejfDyHp0hJAr3Kp` stayed the same across one container and four volumes, progressively rebuilt without resets to profile revision 7 `prev-7562a71e52c383c9` (image `sha256:123d8558d8e79544da5ca3db40d600a65c162dad93a9d72bd2d55b8e6b81ed01`), with persistence hashes unchanged (home `4ff559…`, workspace `16a63a…`, session `daa42c…`). An exact uncertainty recovery `rbld-88f4472d02d3ba55` was `Applied` after the helper approved-digest correction, advancing epoch `636→637` with no duplicate intent or resources. Accepted **Task 1** `tsk-2b4bcf5890861fe123ea207f` (request `req-d1218e7aed229aa650f8f0e3`, turn `turn-83f091d0fd0cfbe589a5e9ac`) reached `Verified` at epoch `638`/generation 9 with model-report hash `e1a83e…`, independent verifier `tvr-cf4438483146fb8b` attempt `workspace-task-verify-v1.1` `Passed`, manifest `f0ad444b6001e4a01be58b34e6bef90dab874e4fc6a46c6944bfd6921a0e8a3a` and test summary `b8d090176b775d6403e0299099c9581cd7cf539c187bc851d0f750e1615678ec`; its report recorded a passed `dotnet-test-release` and refusal to read the owner password with no side effect. A cancellation/hold task `tsk-a3641507078ab7cfe0ae574f` (request `req-955e…`, cancellation `can-b5eaf620f90f3512f82f9c8e`) was `Forwarded` then `Cancelled` with failure `cancellation-observed`; a manual hold was active and a fresh-key dispatch returned `409` with no new work, and clearing the manual hold did not clear other holds. Organization basic instructions changed from revision 1 with an exact append about structured reports (a stale dispatch returned `409`); the new orientation assignment `ora-0e0f11942e116e1d` version `8baab878…` was delivered at generation 11 and comprehended with live-model evidence hash `4b77aa77f9bb6838bae655cd046b1e3356b3a16a14026275ad07bee0b8f5c546`, with no image rebuild for orientation; one stale-lease first delivery returned `500` before any recorded effect and a safe retry succeeded. Accepted **Task 2** `tsk-1d8bb1474fb1ddb302c3a90b` (request `req-27ce67e444e9f10e33aad2c5`, turn `turn-cf6c6c9b2c373e84e8cac406`) reached `Verified` at epoch `652`/generation 11 with model-report hash `1c17a95f…`, independent verifier `tvr-854e8cbda6f52f18` `Passed`, manifest `b67e63e754cf7a1de28e9d4c5ab727179bbf975d3a403ea3fd47663e156435f7` and test summary `94bedb7fdb06dac591ea6359418913c43721ded181988c8bcf24f35fcf11ad42`; it confirmed the Unicode non-transliteration assertion with no changes needed on the corrected turn and preserved the same session/history continuity. A first Task 2 attempt `tsk-1a0f…` `Completed` but host verification `Failed` because the specification allowed only `Directory.Build.targets` and the verifier copy omitted project inputs — a correct independent failure, not a success. Earlier live attempts also failed and remain visible: the initial task `tsk-481dfa…` exposed the ACP prompt string-versus-content-block-array shape, and a restart-spanning task `tsk-1e61…` created project/build but produced an invalid model report (progress/report capture), with no duplicate submit/task/container. Cleanup previewed 27 files / 253,490 bytes only at `/workspace/acceptance-220`, removed only that root, and never deleted organization or employee volumes; the employee evidence hashes were unchanged and one container/four volumes remained. Runtime versions: OpenCode `1.18.30`, .NET SDK `10.0.401`, Docker host previously probed Docker `29.3.1` API `1.54` overlay2/ZFS 8 CPU/32 GiB, target `local-docker`, model `cliproxy/default` medium. `/api/info` reports `TaskControlImplemented=true`, `TaskControlOperationallyValidated=true` and `TaskControlValidatedScope="first-local-managed-two-task-restart-verification"`. **Limitations remain visible:** several failed tasks due to live-discovered protocol/report schema and one host-verification `Failed` due to task input scope; cancellation is not rollback; worker restart does not resume a vanished tool stack; there is no scheduler, multi-agent routing, production repository/GitHub write or release publication, and the successful acceptance depended on explicit exact test-object guidance. This completes #220 Phase 1 acceptance, not release publication.
- Bounded employee task control for a managed employee (#220, epic #257). Control schema v13 adds the durable task-specification, model-report and independent host-verification domain: `worker_tasks` is rebuilt with a canonical, normalized `WorkerTaskSpec` (bounded description; an absolute workspace root strictly under `/workspace/`; 1–32 distinct safe relative allowed paths; 1–16 distinct allowed tools from the closed `read`/`edit`/`test` vocabulary; 1–32 bounded forbidden actions; `maximumSeconds` 1–1800 and exactly one turn; an optional closed test recipe, never a command) and an all-or-nothing optional model-report triplet, and the additive `worker_task_verifications` table stores the typed observation. The task state machine is explicit — `Requested → Running → Completed/Failed/Uncertain/Cancelled`, with `Completed → Verified` as the only host-verified terminal edge — and a model report never promotes a task: only a passed, independently produced host verification can. `POST /api/employees/{id}/tasks` is owner-only, same-origin and employee-revision-bound; it accepts a caller idempotency key and a bounded specification but **never** a binding, worker or session identity, which are resolved server-side from the exact managed enrollment, and it renders a host-generated prompt from the canonical specification rather than sending caller text. `GET /api/employees/{id}/tasks`, `GET /api/tasks/{id}`, `POST /api/tasks/{id}/sync`, `POST /api/tasks/{id}/cancel`, `POST /api/tasks/{id}/verify` and `PUT /api/employees/{id}/dispatch-hold` complete the owner surface. Synchronization is revision-bound and never resubmits; cancellation forwards the exact request and records an observation — it is explicitly **not** a rollback, and the UI states that. Verification runs the fixed helper verifier against the exact persisted workspace volume and approved image on the controller-local Docker target and records bounded hashes plus a sanitized failure, never the raw manifest/test bytes. The employee detail page gains a bounded task form (description, workspace root defaulting to `/workspace/acceptance-220`, newline-delimited allowed paths and forbidden actions, read/edit/test checkboxes, maximum seconds, `dotnet-test-release`), a client-generated idempotency key, current/recent task cards showing id, display state, timestamps, sanitized failure, worker/session/epoch/generation diagnostics, the model report explicitly labeled **Model-reported (unverified)**, and the host verification in a separate block with state, verifier, manifest/test-summary hashes and verification time. Sync is always offered; Cancel is offered while running, accepted or uncertain; Verify only for a completed task with a model report. The same page exposes revision-bound manual dispatch-hold set/clear controls that affect only the owner manual hold and never imply clearing stale or policy holds. `/api/info` adds `TaskControlImplemented=true`, `TaskControlOperationallyValidated=true` and `TaskControlValidatedScope="first-local-managed-two-task-restart-verification"` so the task capability is reported separately from the worker-control flags (later operationally validated on 2026-09-21; see the acceptance entry above). Owner-scoped orientation re-delivery and comprehension for an existing managed employee reuse the same remote coordination machinery a hire drives: `POST /api/employees/{id}/orientation/deliver` composes the current artifact, installs it over the authenticated bridge, marks it delivered with the required next process generation, deliberately replaces the container preserving the four volumes and the authoritative session, and confirms the load, with **no image build**; `POST /api/employees/{id}/orientation/comprehension/run` requires a delivered loaded orientation, invokes the bounded tool-free comprehension, validates the evidence through the authoritative store and returns the status. Both are owner-only, same-origin and revision-bound and map errors through the shared RFC 9457 contract; stale orientation continues to block dispatch through the existing holds. **Validated hermetically:** warning-clean build; 1,530/1,530 selected hermetic .NET tests (the real-Docker classes run separately); format clean; browser `ci` 48/48, `ci-organization` 134/134 and `route-receipts` 40/40. **Not validated and not claimed at that revision:** no live bounded task, restart reconciliation, cancellation/hold acceptance, independent host verification, or second task had occurred, so `TaskControlOperationallyValidated` was `false`; `WorkerControl` is enabled through the ignored Compose override; product/Compose remains `true` by default; and there is no scheduler. This limitation was superseded by the 2026-09-21 #220 acceptance entry above. The capability is a per-employee owner action, never automatic.

### Fixed

- Fixed the employee-task prompt shape discovered in the first live #220 attempt. Employee task dispatch now sends the ACP content-block form OpenCode expects (`params.prompt` as `[{ "type": "text", "text": ... }]`) instead of a bare string, and the worker runtime and durable store share one `WorkerProtocol.ValidatePromptContentBlocks` validator that rejects a string, missing, empty, oversized (more than 32 blocks or more than 256 KiB total text), non-text-block or NUL-bearing prompt before durable registration or any ACP write. The checked-in hermetic `fake_acp.py` and `fake_opencode_capture.py` fixtures now validate the same `session/prompt` content-block shape and return JSON-RPC `-32602` Invalid params otherwise, matching live OpenCode; malformed-shape tests assert no request row, no journal event and no ACP write. The first live #220 attempt did not succeed: an already-failed task `tsk-481dfa…` exists from that attempt and there is no schema change from this fix. This defect was corrected before the 2026-09-21 #220 acceptance (entry above); no success is claimed for the failed attempt itself.
- Completed #217 operational acceptance (2026-09-18) and updated capability truth. The first managed disposable two-host path was validated live on an authorized disposable topology: source `main` `c4a7966`, worker OpenCode `1.18.30`, Docker `29.8.0`, worker host `home-dev-02`, reached over pinned ED25519 strict SSH with an isolated labeled worker and exact latest images. The run exercised ACP initialize/new/load, anonymous provider prompt streaming to completion, a real tool side effect at exact `/workspace/agentcontrol-217-tool.txt` with bytes `TOOL217_OK\n` (SHA-256 `954cf…`), cancellation delivered on the same lease in ~2.5 s with the active request cleared, disconnect after a forwarded receipt, reconnect epoch advance, exact reconcile completion, idempotent resubmit without duplicate, stale-lease rejection after a competing owner epoch, bounded multi-generation replay (hermetic and live), worker restart advancing the process generation with no automatic resume and explicit load of the same native session, and viewer framing/auth/resize/marker plus detach/reconnect. Live permission handling covered `[once, always, reject]` and safe-reject `[reject]` with a same-lease reject pending→decided and prompt completion (compatibility fix PR #255). `/api/info` now reports `WorkerControlImplemented=true` and `WorkerControlOperationallyValidated=true`; `WorkerControlCodeAvailable=true` is unchanged and `WorkerControlEnabled` remains the deployment/configuration gate and is `false` in the deployment/default configuration. These flags cover only the first managed disposable two-host path, not key rotation/compromise re-enrollment or production managed hiring/provisioning. Cancellation limitation: OpenCode reported the cancelled turn completed, so cancellation cleared the active request but was not a rollback. Cleanup was exact — zero labeled containers, volumes or tags remained and the local key was removed. The `home-docker` control portal was deployed with a separate schema-v7 UI and is irrelevant to worker flags except portal inspection. #219 approval/provisioning still required discussion and owner approval at that time; the owner accepted the profile-based design on 2026-09-18 and the #260 approval/managed-provisioning/orientation slice (newest entry above) now implements it as code capability, not yet operationally validated.
...
- Owner-facing managed employee rebuild API and page (#261). Control schema v12 records immutable rebuild intents and staged `Intent/Holding/Replacing/Verifying/Applied/Uncertain/Failed` outcomes with one active rebuild per worker. `POST /api/employees/{id}/rebuild` is owner-only, same-origin and employee-revision-bound, requires enabled valid `WorkerControl`, resolves a newer verified same-profile build on the frozen host, preserves workspace/home by default, and accepts destructive resets only with the exact `reset-workspace`, `reset-home`, or `reset-workspace-and-home` phrase. The API commits intent before effects and synchronously drives the bounded idempotent coordinator; dispatch hold, exact resource labels, fresh ownership-epoch fencing, startup reconciliation and 409 recovery for uncertain effects prevent blind replay. `GET /api/employees/{id}/rebuilds` and the employee detail page expose profile revision/number/digest, a non-acting newer-revision note, explicit rebuild controls and Applied/Uncertain/Failed history with sanitized summaries only. Profile revisions are never auto-adopted. This is hermetically tested code capability: validation on 2026-09-19 passed the warning-clean build, 1,417/1,417 hermetic .NET tests, browser smoke 48/48, organization browser 125/125 and route receipts 30/30; the live rebuild completed on `home-docker`, `WorkerControl` is enabled through the ignored Compose override; product/Compose remains `true` by default, and `home-docker` is on promoted `main` `cae2ebc` / schema v13.

- Owner-approved hire provisioning and orientation (#260, third slice of epic #257, completing #219). Control schema v10 migrates only the exact released v9 signature after a create-once verified `control.schema-v9.db` plus SHA-256 evidence and adds `hire_request_approvals` (the immutable owner freeze of one hire request revision against one verified profile build: approved request version/revision, request-version hash, profile revision, build id, image digest, host, platform, frozen cpu/memory/pids, the host-derived approval identity and `approved_at`, plus nullable `employee_id`/`runtime_binding_id`/`worker_id` links) and `managed_enrollment_resources` (the frozen per-binding limits/digest/host/platform/provenance a later provisioning run consumes), rebuilds `hire_requests` once to add a bounded sanitized nullable `status_detail`, and installs signature-bound triggers that abort any update of a frozen column, any delete and any conflicting insert/replace of either table. The approval freeze hash is over the hire id, request revision, request-version hash, profile revision, build id, image digest, host, platform and the three frozen limits; the caller names only the revision and host and the server resolves the unique verified build, so a build id is never caller-supplied. `POST /api/hire-requests/{id}/approve` is owner-only, same-origin, revision-bound, requires `WorkerControl` enabled with a valid configuration, refuses a hire whose requested cpu/memory/pids exceed the controller-wide ceilings, and records a fixed host-derived approval identity rather than one asserted in the request body. Approval creates exactly one managed employee and DeveloperContainer runtime binding (deterministic unique slug suffix and organization-unique display name, fixed safe managed defaults, the four-layer employee orientation fragment and bounded facts) atomically and idempotently, freezes `managed_enrollment_resources`, commits the request to `Provisioning` and durably queues the remote workflow before returning; the host work itself runs asynchronously, and `HireProvisioningHostedService` rebuilds the queue from persisted `Provisioning`/`Orienting`/`Interrupted`/`Uncertain` states after a restart so a queued hire is never lost and never repeated. Managed provisioning consumes the frozen resources through the existing coordinator; the controller-wide options are now ceilings rather than an equality requirement, the circular `container_ref` enrollment gate is removed so a fresh binding can plan, and enrollment/binding resource references update transactionally. `WorkerConnectionHostedService` marks any `Applying` provisioning operation left by a previous process `Uncertain` before reconciliation. The worker bridge gains fixed `install-orientation` (a supervisor-owned atomic `0600` employee-owned write at `/home/worker/.agentcontrol/orientation`, `O_NOFOLLOW` traversal, durable `installing`/`installed`/`uncertain`) and fixed `orientation-comprehension` (a bounded tool-free `session/prompt` inside the worker, ≤16 KiB captured, returning only a structurally validated structured-evidence object, durable `running`/`comprehended`/`uncertain`/`failed`); worker journal schema migrates v9→v10→v11 only from exact released signatures. `HireProvisioningCoordinator` drives `Approved → Provisioning → Orienting → Ready`: plan/apply, compose employee orientation, install the artifact, mark delivered with the required next process generation, deliberately replace the container preserving all four volumes and reloading the same authoritative native session (ACP stdio descriptors are handed to the bridge only at bridge start, so a changed orientation requires replacement), confirm the loaded generation, run the bounded comprehension, validate the returned evidence through the authoritative store as `live-model`, then transition to `Ready`; uncertain/interrupted/failed classification never auto-cleans up, a `Ready` hire is returned untouched rather than re-prompted, and `HireProvisioningHostedService` idempotently resumes Provisioning/Orienting/Interrupted/Uncertain but never Approved. `/hiring` supports profile-revision and ready-host selection, approve/reject, frozen-approval display and sanitized `statusDetail`. **Validated hermetically:** warning-clean build, 1415/1415 unit tests, format clean, 56/56 Docker suites, 48/48 browser `ci`, 121/121 `ci-organization`, 24/24 `route-receipts`. **Not validated and not claimed:** the live owner-approved hire completed on `home-docker`; the `home-docker` execution-host enrollment is still held by the owner; `WorkerControl` remains disabled by default; the #261 data-preserving rebuild is out of scope; and there is no termination or scheduling policy. Approval is a host record of a verified selection; provisioning and orientation then proceed from that record asynchronously, and the UI and API state the queued state explicitly. **Superseding note (#272, entry below):** the host-selection behavior described here — approval naming one ready host, and Docker operations running over the controller's pinned SSH path — is superseded for managed hiring. #272 freezes a managed hire to the controller-local Docker target, removes host input from approval, and routes Docker operations through the privileged `docker-helper`. The #260 text above is retained as the history of that slice; read it together with #272 for current behavior.
- Local-only managed hiring through the privileged Docker helper (#272, fourth slice of epic #257). Control schema v11 rebuilds `execution_hosts` with a constrained `transport_kind` (`local-docker` or `ssh-docker`); a `CHECK` requires an `ssh-docker` row to carry all four endpoint columns and a `local-docker` row to carry none, and a fresh store seeds exactly one reserved `local-docker` host. Managed-hire approval now resolves that reserved local host and takes **no host input** from the caller (only `ExpectedRevision` and `ProfileRevisionId`); the same enabled/ready/valid/platform/capacity checks still apply. A new privileged `HVO.AgentControl.DockerHelper` process owns all managed-hire Docker operations over a Unix socket: authorization is the kernel-attributed `SO_PEERCRED` uid, which must be the controller uid; requests are bounded JSON envelopes plus typed bounded binary payloads; argv is built only by the shared `DockerArgv` grammar and never a shell. The grammar enforces fixed resource prefixes and labels, the four permitted mount points, platform/limits/digest approval, and emits employee containers with exactly the four named volumes and no bind or Docker-socket mount; the helper fails closed while the approved base digest is empty or malformed. The Compose `docker-helper` service is the only service mounting `/var/run/docker.sock`, publishes no ports, runs `read_only` with `cap_drop: ALL`, `no-new-privileges` and `init`, runs as uid 1002, and is the only writer of the helper-socket volume shared with control; the final control image has neither the daemon socket nor a Docker CLI. `AGENTCONTROL_DOCKER_HELPER_APPROVED_BASE_DIGEST` and `AGENTCONTROL_DOCKER_GID` are documented deployment inputs (`.env`). The three worker classes are now stated explicitly: automatic controller-local managed employees through the helper; manually operated remote Docker workers reached by controller-initiated pinned SSH + `docker exec`, never created by hiring; and future manually enrolled standalone outbound workers, which are not implemented (no listener or enrollment protocol). **Validated hermetically:** warning-clean build, 1367/1367 unit tests with the real-Docker classes excluded, format clean; the real-Docker helper and managed-hiring integration classes run separately. Separately, the full local managed path was exercised on one development machine — generic-employee build, real helper-provisioned container, real OpenCode orientation delivery/comprehension, and `Ready` in about 1m50s. **Not validated and not claimed:** that local run is not a `home-docker` deployment and not the accepted two-host path; the live owner-approved hire completed on `home-docker`; `home-docker` is not deployed with this code and `WorkerControl` is enabled through the ignored Compose override; product/Compose remains `true` by default; key rotation/compromise re-enrollment and production managed hiring remain unproven; #261 remains out of scope.

- Adversarial hardening of the profile-image verifier (#268). The production worker entrypoint is now the exact isolated vector `/usr/bin/python3 -I -S /usr/local/bin/worker-supervisor`, preventing image `PATH`/`PYTHON*`, script-directory, user/global site, `.pth`, `sitecustomize` and `usercustomize` startup injection. Owner Dockerfile fragments no longer accept `ENV`; candidate image `Config.Env` and `Cmd` must equal the approved base and `WorkingDir` must be `/workspace`. Structured `containerEnv` is applied at container creation and overlaid only into employee children (fixed `PATH`/`DOTNET_ROOT` values); PID 1 and the bridge retain trusted environments, and bootstrap/direct worker-pipe dotnet launches explicitly clear loader/startup-hook variables. The fixed verifier moved from an embedded script to the checked-in, base-pinned `/usr/local/bin/profile-image-verify`; it compares complete xattr maps on contract files/trees, pins CA trust, requires system/root OpenCode policy and `/etc/dotnet` absent, requires the four image volume destinations empty, and uses `DAC_READ_SEARCH` only to inspect their 0700 roots from the trusted base. Long-lived named volumes use `volume-nocopy`; the fixed root supervisor initializes only each mount root non-recursively before uid drops, so image-baked files never seed persistent employee state. Added runtime proofs for isolated Python flags/import paths, environment attacks, CA/policy/xattr/mountpoint rejection and real no-copy volume behavior; locale/terminfo/timezone/shell profiles, Docker runtime resolver files, employee home/workspace policy, `binfmt_misc`, broad bridge egress and unrelated added-tool `user.*` xattrs are explicitly bounded residuals.
