...
- `home-docker` is deployed from promoted `main` `cae2ebc` at schema v13 for the #220 acceptance; `quick_check` is clean and FK errors are zero. The task-control capability flags in that currently deployed `cae2ebc` build are still `TaskControlOperationallyValidated=false` / `TaskControlValidatedScope=null`; `/api/info` reports the accepted task flags only after the later evidence branch commits are promoted. WorkerControl is enabled through the ignored Compose override, while the product/Compose default remains `true`.
