...
- First owner-facing #219 slice. The static SSR portal is now split into real authenticated routes (`/organization`, `/employees`, `/employees/{id}`, `/hiring`, and `/system`) under a persistent responsive layout; route responses no longer ship every page section and hide them behind hash navigation. The all-employee directory supports search and department/availability filters, stable detail links, and a clear “Request employee” path. Exact employee detail alone loads orientation actions and terminal/model controls. Control schema v7 migrates only the exact frozen v6 signature after a verified create-once `control.schema-v6.db` plus SHA-256 evidence, adding durable idempotent hire requests and append-only hash-only events. Same-origin owner APIs support create/list/get and revision-bound rejection with bounded purpose/name/resources and organization/department/role relationship validation. Approval is deliberately absent and the UI truthfully disables it: #217 two-host operational acceptance must complete before approval, provisioning, orientation, or employee creation. The organization section is now a real hierarchy: `/organization` renders cards linking each department by stable ID, `/organization/departments` is the authoritative directory, and `/organization/departments/{id}` shows the department revision, persisted standing instructions, assigned roles, scoped roster with availability badges, availability counts, scoped failures, an empty-state message, and a `Request employee` path that preselects the exact department in `/hiring?departmentId=<id>`. `GET /api/departments/{id}` is owner-authenticated and applies the shared stable-id validation: invalid ids are `400`, unknown ids `404`, and a forged department/role relationship is still rejected by the create endpoint. The shared navigation groups Overview and Departments under Organization with a server-computed parent active state.
