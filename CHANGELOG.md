...
- `home-docker` is deployed from promoted `main` `7d4078b` at schema v12; `quick_check` is clean and FK errors are zero. WorkerControl is enabled through the ignored Compose override, while the product/Compose default remains `true`.
...
- #220 is now operationally validated (2026-09-21): the first local managed employee (`emp-933d24fc110222a5`) ran two bounded tasks end to end and each reached `Verified` through independent host verification on the controller-local Docker target, across a control restart, a cancellation/hold exercise, an orientation revision and an exact uncertainty recovery. `/api/info` reports `TaskControlOperationallyValidated=true` and `TaskControlValidatedScope="first-local-managed-two-task-restart-verification"`. There is no scheduler; cancellation is not rollback; there is no multi-agent routing, production repository/GitHub write or release publication.
