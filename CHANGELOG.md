...
- `home-docker` is deployed from promoted `main` `59d0d68` at schema v13 (the #220 evidence commit `34370f2` promoted through PR #318); `quick_check` is clean and FK errors are zero. The live `/api/info` reports `TaskControlImplemented=true`, `TaskControlOperationallyValidated=true` and `TaskControlValidatedScope="first-local-managed-two-task-restart-verification"`. The worker base digest was unchanged from `cae2ebc`, so only the control container was recreated and the employee kept profile revision 7 with no rebuild. WorkerControl is enabled through the ignored Compose override, while the product/Compose default remains `true`. #220 and #211 are closed.
