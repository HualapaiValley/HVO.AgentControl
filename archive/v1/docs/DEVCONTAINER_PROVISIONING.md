...
The deployed control service now exposes an injectable, fixtureable `IDevContainerCliOperationAdapter` contract. It consumes registered host, project, runtime-environment and selected configuration identities; preserves requested evidence separately from resolved evidence; derives immutable request/host/project/runtime/configuration labels and a canonical digest over every currently modeled immutable input; records requested/resolved/observed DTO evidence; bounds reported output to 4096 characters; and classifies zero, unique, and multiple exact-label reconciliation matches as uncertain, observed, and uncertain respectively. A request ID cannot be reused with changed intent. It is intentionally non-executing: `Execute` returns `missing_provisioner_authority`, never starts the CLI, shell, Docker, a native session, or a container, and does not claim provisioning success.
...
`Provisioning/LocalDevContainerRunner` is an injectable **host-process** implementation of official CLI `read-configuration`, `up`, and `exec`, plus exact Docker observation/removal. It is not registered in the web application and introduces no Docker socket mount there or in a worker. The [durable operation API and ledger](PROVISIONING_OPERATIONS_API.md) now join owner intent to restart-safe effect admission, but no authenticated host executor is registered, so production requests remain held before host authority.
...
The caller also supplies `IProvisionAttemptLedger`. `DbProvisionAttemptLedger` now durably binds operation UUID to the complete intent digest, rejects changed intent after restart, serializes access to the operation and writable workspace, requires internal capacity/host authority, and commits effect-start once before authorizing a process. The runner/SQLite integration test proves reconstruction does not repeat `up`. The file ledger remains a runner-only acceptance fixture. Neither ledger supplies production capacity evidence, authenticated executor delivery, host enrollment or retirement authority.
...
The runner separately validates raw source paths and the actual CLI `mergedConfiguration` before admitting `up`. Local Feature metadata can add privileges, mounts or capabilities absent from the raw config; absent/non-object effective configuration or unsupported effective privileges, user/workspace identity or Docker arguments cannot authorize creation. Resolved Feature sources, including dependencies, must remain local paths in the approved checkout. Raw fields use an explicit allowlist for this profile: image/build, local Features, user/workspace/environment settings, resource arguments and container lifecycle hooks. Build fields are limited to `dockerfile`, `context`, `args`, `target` and empty `options`. Nonempty `build.options`, `cacheFrom`, legacy top-level `dockerFile`/`context`, port publishing and unknown fields are unsupported: host secrets, SSH forwarding, cache imports and additional build contexts require separate authority. Default Docker layer-cache reuse is unaffected. Independent Docker inspection also rejects extra `CapAdd`/`SecurityOpt` or privileged/bind-mount mismatches before execution probes.
...
`RemoveOwned` requires a separate ledger action/effect authorization and the exact single observed container ID. It removes that container only, leaves volumes, checkout paths and Docker image/build caches untouched, and returns the observed retained-resource inventory. Missing or unverified checkout paths are explicitly reported as `workspace-missing-or-unverified`, with removal code `owned_container_removed_workspace_retention_unverified`; their data is never claimed preserved. The future lifecycle caller must establish drain, preservation/backup or explicit loss disposition, and retirement authority first, including container-local state or a bind source that disappeared. It must not wire this method directly to an unauthenticated endpoint or mistake it for the full drain/retire workflow.

The manually dispatched `Dev Container runner` workflow installs the locked official package and runs the checked-in `tests/DevContainerCli/fixture`. It is also selected by `devcontainer` or `all` in **Full validation**; neither workflow automatically repeats it for PRs or merges under the [prerelease validation policy](VALIDATION_POLICY.md). The fixture creates a cold build and an independently owned warm container, observes `vscode`, .NET/Git/bash, a local Feature and postCreate marker, compiles/runs a small .NET project, reconstructs admission for replay, rejects changed intent, exercises a failing hook, and removes exact owned IDs. Two additional checkouts prove actual CLI merged Feature elevation and ignored Feature inputs are rejected before creation. The trusted acceptance caller discovers installed Node/Git/Docker paths, including hosted CI tool-cache installations, then binds their canonical paths into its authority. Checkout and image-cache retention are reported. Cleanup continues across per-owner failures and always writes a receipt; uncertain removal/ownership retains the fixture root rather than deleting potentially mounted source.
