...
This describes the first implemented slice of #121 and [the execution environment plan](EXECUTION_ENVIRONMENT_PLAN.md). Its original migration creates three tables and does not infer or rewrite any existing runtime, worker, command, credential, directory, native session or transcript binding. The subsequent [runtime environment configuration API](RUNTIME_ENVIRONMENT_API.md) adds explicit requested associations and reference guards; it does not backfill the fleet. Legacy `Worker.Project` display labels remain display labels. A missing project must be registered with an explicit repository URL; it is never guessed from that label or a working directory.

## Resource contract

`HostRecord` identifies an owner-registered machine/VM independently of an SSH endpoint or Docker container. It contains `id`, `sequence`, `name`, `kind`, `description`, `archived`, `revision`, `createdAt` and `updatedAt`. Kind is owner-declared `Unclassified` (default), `PhysicalMachine` or `VirtualMachine`; it is not verified platform, capacity or Docker authority. Registering a host does not connect to it, make it schedulable, grant credentials, or authorize remote effects. Requested runtime associations are described in [the environment API](RUNTIME_ENVIRONMENT_API.md); verified placement remains follow-up work.
...
Archive only changes inventory visibility while preserving identity and retry/history evidence. Unarchive uses the same endpoint with `archived=false`. There is no DELETE, remote stop, container removal, retirement or purge here. Host archive now rejects requested runtime associations; project archive rejects references as a runtime configuration source, with `resource_in_use` (409). Reset or reassign those references before archiving. Legacy workers/runtimes remain unbound until explicitly configured. Before consumers can schedule from these records, they must enforce archive eligibility and the binding/ownership rules below; hiding a host is not draining its workers.
...
| Requested runtime kind/host/configuration-source association and managed one-registration default | Implemented in the subsequent [runtime environment slice](RUNTIME_ENVIRONMENT_API.md) |
| Verified host/container binding; logical worker slot; legacy conversation binding; project-qualified task/workspace/session identities | Remaining #121 work; no inferred hosting or session backfill |
