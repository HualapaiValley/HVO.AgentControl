...
Owner-reported local capacity: an **M5 Mac with 128 GB unified memory already running Ollama**, and an **M4 Mac with 48 GB unified memory**. These are prospective inference hosts; their endpoints, installed models, available memory and sustained throughput have not been verified. Ollama availability on the M4 is not yet confirmed. Do not conflate the reported M4 with an existing runtime record without verifying identity.

A worker can use a reachable inference host independently of where its repository and development tools run. Discover the host's actually served models, qualify each exact model/quantization through the OpenCode adapter, and advertise only validated routes. Unified memory capacity alone does not establish a model's usable context, latency or concurrent request capacity. Track health, queue depth and memory headroom, with a configurable local concurrency limit and capacity reserved for the owner's interactive use. Treat sleeping, disconnected or overloaded Macs as temporarily unavailable; retain their configuration for recovery. Local routes should compete with hosted routes on task success and expected completion time, while preserving hosted allowance when suitable. If availability is lost mid-task, reconcile session/tool state before continuation on another route.
