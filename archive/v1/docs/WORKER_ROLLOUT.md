...
| Activation request ID is reused with a different project/binding/head, or an activation arrives after owner abandonment | Changed intent returns a conflict rather than another task's old receipt. A late result cannot unarchive the abandoned task's worker or admit a prompt. Include a valid unchanged-request replay and valid activation as positive controls. |
...
| A bound task prompt supplies `GitHubMergeScope` for the other repository | Match the typed scope to the binding's project repository both before enqueue and immediately before dispatch. The wrong scope produces zero native HTTP submissions; the correct scope still succeeds. Runtime-wide repository allowlists alone do not establish this task binding. |
...
## CLIProxy test provider

The opt-in `compose.beta.cliproxy.yaml` overlay supplies the same CLIProxy catalog used by the control sidecar through a protected Docker secret. It mounts `docker/development/opencode-cliproxy.json` at OpenCode's standard per-user configuration path. This is deliberate: SSH-launched tmux sessions do not inherit Docker environment variables, so `OPENCODE_CONFIG` alone is not reliable for a worker. The default is the free `cliproxy/big-pickle` model and the catalog contains only models observed from the CLIProxy endpoint. GPT model IDs are intentionally omitted while the owner has development workers restricted to non-GPT routes.

Applying the overlay requires `CONTROL_CLIPROXY_API_KEY_FILE` to point to a protected local file. Do not copy the key into the repository, image, command line, event journal or worker prompt. Set the overlay on a quiet worker or a newly provisioned worker first; changing the OpenCode configuration requires restarting that native OpenCode process and can interrupt an active turn. After the restart, refresh the workspace model catalog, verify the provider/model in the durable execution payload, and run one bounded inference before changing additional workers. Existing queued payloads retain their original provider and model.
