...
The predecessor remains the ordinary scope lookup result while its successor is pending or ready. After a paused coordination run migrates to that direct successor, the successor becomes current and a new ordinary create UUID resolves it; replaying an older create UUID still returns that UUID's original binding. The predecessor worker is retained as an archived, read-only conversation. Migration requires both sessions to be freshly observed idle, all delivery to be resolved and interactive requests to be settled. It keeps the original run, round, task assignments and effect receipts and leaves the owner-paused run paused. Renewal does not resume a run.

Repeating an ordinary create request for the same scope/settings discovers the current generation with a separate discovery receipt; it does not renew the conversation or change current model settings. Edit a current bound session through the page or existing worker settings endpoint using its compatibility `workerId`. Connection refresh/disconnect and provider endpoints likewise use the service ID as the internal runtime ID. API responses retain explicit provisioning and delivery states. This generation API is an explicit lifecycle foundation: it does not rotate on a token threshold, infer model context limits, compact or delete native history, or override an owner pause/stop. A later context policy must use adapter-verified effective-input semantics, including cache accounting where applicable, rather than treating raw input-token counters as the whole context.
