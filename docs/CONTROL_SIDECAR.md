...
`ControlServiceRecord` owns endpoint and native instance identity; `ControlSessionBinding` owns the stable scope, generation, creation receipt and native session identity. Every successor generation has a new binding, worker projection and native session, so predecessor transcripts and pending requests keep their original identity. Internal runtime/coordinator records are compatibility projections for command delivery and existing conversation UI, not development runtime registrations. There is one control service per host and at most 64 current scopes. At most two control model turns dispatch concurrently, independently of development capacity. Different workgroups have separate coordination runs; each receives scheduling and heartbeat opportunities, and shared worker claims remain atomic across runs.
...
| Ensure a scope | `POST /api/v1/control-services/{id}/sessions` with request `id`, `scopeKind`, `scopeId`, `name`, optional `providerId`, `modelId`, `variant`, `agent` |
| Retry known rejected creation | `POST /api/v1/control-services/{id}/sessions/{sessionId}/retry` with request `id`, `expectedRevision` |
| Create a successor generation | `POST /api/v1/control-services/{id}/sessions/{sessionId}/renew` with request `id`, `expectedRevision`; returns `successor` plus the durable renewal `receipt` |
...
Valid scopes are `HostOperations` / `host` (created by registration) and `Workgroup` / an owner-defined stable ID. Generation zero keeps its original `agentcontrol-control:<bindingId>` discovery title. An authenticated, CSRF-protected renewal of a current ready Workgroup binding derives the next generation and queues one exact native creation with a generation-qualified title. Repeating the renewal UUID returns its durable receipt and same successor; a changed payload, stale predecessor revision or second successor conflicts. A lost native response is reconciled by that exact title and is never replayed speculatively.

The predecessor remains the ordinary scope lookup result while its successor is pending or ready. After a paused coordination run migrates to that direct successor, the successor becomes current and a new ordinary create UUID resolves it; replaying an older create UUID still returns that UUID's original binding. The predecessor worker is retained as an archived, read-only conversation. Manual migration requires both sessions to be freshly observed idle, all delivery to be resolved and interactive requests to be settled. It keeps the original run, round, task assignments and effect receipts and leaves the owner-paused run paused. Manual renewal does not resume a run.

Repeating an ordinary create request for the same scope/settings discovers the current generation with a separate discovery receipt; it does not renew the conversation or change current model settings. Edit a current bound session through the page or existing worker settings endpoint using its compatibility `workerId`. Connection refresh/disconnect and provider endpoints likewise use the service ID as the internal runtime ID. API responses retain explicit provisioning and delivery states.

When an in-flight coordinator decision exceeds its configured queue-wait, inference, native-retry, compaction or total budget, a healthy current ControlHttp Workgroup binding can allocate one isolated successor automatically. The durable recovery intent records source/successor generations, owner-policy revision, instruction hash, source decision and exact creation command before native effects. `CoordinatorGenerationRecoveryLimit` bounds automatic attempts per scope, `CoordinatorGenerationRecoveryMinutes` bounds each provisioning/cutover interval, and `CoordinatorGenerationRecoveryEffectiveInputTokenLimit` rejects known source input plus cache spend above its configured ceiling. Recovery requires a fresh exact user caller in the expected native session and a fresh observation of zero child sessions. SSH, changed session/caller/process fences, history gaps, observed tools or children, pending interactive requests, competing commands, unhealthy services and exhausted limits retain the explicit recovery hold instead. `EnableCoordinatorGenerationRecovery` defaults to false; enable it only after the pinned-native canary passes in the target deployment.

Automatic recovery never calls generic session Abort. Native creation is authorized again immediately before its POST; owner pause/stop/follow-up or a changed fence cancels an unsent creation. A sent creation response that is lost remains Unknown and is discovered by its original title without another POST. After a ready successor is freshly observed idle, C# atomically makes it current, archives the predecessor projection and retires only the old proposal's routing authority. The old command and native delivery remain truthful and auditable. One linked replacement decision is then built from current owner instructions and worker evidence. A late old reply cannot dispatch actions, and unrelated development commands are unchanged. If the old proposal finishes before cutover, its still-current authority can be validated normally and an unsent successor creation is cancelled.

This policy is driven by measured decision-phase time, not a guessed token threshold. It does not infer model context limits, compact or delete native history, or override owner pause/stop. A later proactive context policy must use adapter-verified effective-input semantics, including cache accounting where applicable, rather than treating raw input-token counters as the whole context.
...
After starting the independent sidecar, the first opt-in deployment saves its network name in `.fixture/control-network-name`. This file contains one Docker network name and no credentials. `scripts/demo-server.sh` reads it on every start/restart, includes the overlay, and runs the network helper before showing successful web status. Keep the usual `DEMO_BIND_ADDRESS` setting. For an existing state directory used from a fresh source checkout:

```bash
export HVO_DEMO_STATE_ROOT=/path/to/existing/.fixture
# First opt-in only; refuse to replace an existing setting.
(set -o noclobber; printf '%s\n' 'hvo-agentcontrol-control_control' > "$HVO_DEMO_STATE_ROOT/control-network-name")
bash scripts/demo-server.sh start
bash scripts/demo-server.sh restart
```

Without a marker or existing labelled web deployment, the script retains the original setup. For compatibility, it can discover an existing web container's opt-in label; it persists that name only after the helper verifies the matching owned network. Invalid markers fail before a restart stops the web host. A configured but unavailable sidecar/network fails deployment without reporting successful status. Stop/restart explicitly target `agentcontrol`, leaving the legacy coordinator profile and independent sidecar running. `HVO_DEMO_STATE_ROOT` defaults to the source checkout's `.fixture`; setting an absolute existing path keeps persistent data and secrets there while building code from the checkout containing the script. The script supplies `DEMO_DATA_DIRECTORY` and `DEMO_SECRETS_DIRECTORY` to the configurable Compose mounts.

Other release workflows must likewise retain the overlay and invoke the helper automatically after **every** web create/recreate and before declaring deployment ready. A standalone manual `docker network connect` is insufficient because Compose recreation removes that attachment. The helper accepts `--expected-container-id` and `--expected-network-id` for deployment identity checks; these are Docker IDs, not credentials. Custom Compose project names use `--project` and `--control-project`; the demo script forwards `COMPOSE_PROJECT_NAME` for its web project. Set `CONTROL_OPENCODE_NETWORK` to the actual control network name when directly rendering the overlay. The helper never creates or deletes a network, changes an SSH worker, stops the sidecar, or publishes its port. Membership verification is followed by the application's own authenticated service-identity and readiness checks; network membership alone does not establish native service continuity.

Run `python3 tests/control_web_network_smoke.py` to exercise this transition with isolated disposable resources. It verifies legacy worker IP access and `opencode-control` DNS access from the same web container, the published loopback UI port, idempotence, web recreation followed by reconciliation, unchanged sidecar process identity, and rejection of missing labels/wrong identities without changing unrelated memberships. `python3 tests/demo_lifecycle_smoke.py` invokes the actual demo script with recorded command fakes to cover enabled/disabled starts and restarts, verified fallback persistence, invalid/missing requirements, external state paths and web-only lifecycle targeting. Tiny HTTP services stand in for the applications; these fixtures do not claim real model inference or a live deployment. A later migration of all worker addresses to managed user-defined networks can replace this transitional helper with ordinary Compose network declarations.

## Restart and migration behavior

Host startup reloads existing mappings and verifies the same native instance. Every direct connection verifies native health, pinned version and schema before becoming available, including connections used for provider delivery. Periodic checks revalidate health and version with a bounded response; a changed incarnation must also pass schema verification before observation or dispatch resumes. A web restart preserves the independently running native process. A changed sidecar incarnation records an explicit restart observation; session history is retained but interrupted computation is not claimed to have resumed. Ordinary uncertain instructions are reconciled without replay. For an exact, still-unapplied coordinator decision, all native tools were disabled: the service retires that proposal's routing authority with a receipt preserving **unknown native delivery**, then schedules a fresh decision against current evidence. This also handles backend startup converting a dispatching proposal to unknown. Owner-paused/stopped runs remain paused/stopped. The old proposal is never applied or resent.
