...
Before every `up`, the runner verifies tools, source and Docker engine and inspects the operation's actual Docker ownership. It queries by operation first and validates every expected label after inspection, so a conflicting digest cannot become a false zero match. One exact owner is observed and verified without repeating `up`; multiple/malformed/mismatched owners remain unknown. Once an up attempt is recorded, zero observed containers also remains unknown and cannot authorize another up. A caller must explicitly reconcile and design a separate recovery operation; this runner does not reset attempts or use `--remove-existing-container`.
