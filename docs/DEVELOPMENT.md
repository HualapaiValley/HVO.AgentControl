...
| Hire approval (`POST /api/hire-requests/{id}/approve`) | Not implemented | Owner-only, same-origin, revision-bound approval that freezes one hire against one verified profile build on a ready host and atomically creates the managed employee identity and DeveloperContainer binding; it does not provision or orient. Code capability with hermetic coverage only — no live owner-approved hire has run and `WorkerControl` is off by default |
