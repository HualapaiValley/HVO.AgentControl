...
3. Create the App, note its **App ID**, and generate/download its PEM private key. Install the App on selected repositories, initially only `RoySalisbury/HVO.AgentControl`. The installation settings URL contains the **installation ID**; this is different from the App ID or client ID.
4. In AgentControl's **GitHub** page select **Devcontainer .NET test**, enter the App ID, installation ID and `RoySalisbury/HVO.AgentControl`, and paste the PEM into the private-key field. Submit **Verify and configure access**. The field is cleared after submission. Never put the key in an agent prompt or chat.
5. Keep the runtime connected. The background service delivers credentials over its pinned SSH connection and shows **Ready** or a visible **Blocked** reason. Existing personal GitHub configuration is not overwritten. All sessions using this runtime's OS account share the grant.
