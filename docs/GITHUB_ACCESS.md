...
The `/github` owner page configures GitHub App access for an execution runtime. AgentControl stores the App private key encrypted and issues installation tokens restricted to the selected repositories, Contents/Issues/Pull requests write permissions, and available Checks/Commit statuses/Actions read permissions. It verifies the installation account, returned repository set, effective permission set and expiry before delivery. The private key never goes to a worker.
