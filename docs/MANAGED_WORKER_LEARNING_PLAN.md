...
### Terminal evidence budget fallback (#7)

The live coordinator twice exhausted the 64,000-character prompt budget despite existing per-field truncation. A long owner instruction plus 16 completed reports and JSON escaping can exceed that budget. After ordinary compaction, the context builder may replace older terminal report bodies with explicit omission receipts. It preserves their command/worker identity, delivery state and dispatch ledger, every nonterminal result, and each worker's newest terminal response. Owner instructions, requests, routing inventory and stored evidence remain unchanged.

The omission marker explicitly says missing evidence is not approval/completion; the coordinator must retrieve or request a concise exact report before relying on it, without repeating its side effects. Oversized mandatory routing facts still pause safely. This is a bounded liveness fix, not the durable per-consumer cursor/retrieval workflow still required by #7. Regression coverage includes heavily escaped historical reports, preserved active work/latest findings and unchanged durable results.
