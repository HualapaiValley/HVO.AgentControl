...
`scripts/agentcontrol-watchdog.py` supplies a small operational monitor outside the controller process. It authenticates using the owner password file, refreshes an expired login on the next poll, and reads the snapshot and coordination ledger with ten-second request timeouts and an 8 MiB response limit. Run it against the controller's local interface or an authenticated HTTPS origin. The password stays out of arguments, logs, and the incident file.
