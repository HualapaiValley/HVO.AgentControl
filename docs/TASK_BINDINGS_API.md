...
- `TaskSessionBindingRecord` is an explicit native-session association. New bindings begin `Unbound`; this API never creates a native session. A legacy session can be attached only when the caller supplies both the existing worker ID and its exact recorded native session ID. No legacy rows are backfilled.
