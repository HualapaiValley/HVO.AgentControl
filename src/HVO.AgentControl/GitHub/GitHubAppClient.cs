...
public sealed partial class GitHubAppClient(HttpClient http, TimeProvider clock)
{
        ...
        IEnumerable<string> repositories, CancellationToken cancellationToken)
    {
        ...
        var account = installation.RootElement.GetProperty("account").GetProperty("login").GetString();
        var slug = installation.RootElement.GetProperty("app_slug").GetString();
        ...
        var requested = new { repositories = scope.Select(x => x.Split('/')[1]).ToArray(), permissions = new { contents = "write", issues = "write", pull_requests = "write" } };
        ...
        var granted = root.GetProperty("repositories").EnumerateArray().Select(x => x.GetProperty("full_name").GetString() ?? "").ToArray();
        ...
        if (string.IsNullOrWhiteSpace(token) || token.Length > 8192 || token.Any(char.IsControl) ||
            ...
            !root.TryGetProperty("repository_selection", out var selection) || selection.GetString() != "selected" ||
            !new HashSet<string>(scope, StringComparer.OrdinalIgnoreCase).SetEquals(granted) ||
            new[] { "contents", "issues", "pull_requests" }.Any(x => !permissions.TryGetProperty(x, out var value) || value.GetString() != "write") ||
            permissions.EnumerateObject().Any(x => x.Name == "metadata" ? x.Value.GetString() != "read" :
                x.Name is not ("contents" or "issues" or "pull_requests") && x.Value.GetString() != "none"))
