...
public sealed partial class ControlStore
{
    ...
    private async Task<ControlSessionBinding> QueueControlSession(ControlDb db, ControlServiceRecord service, CreateControlSessionInput input)
    {
        ...
        if (input.ProviderId is null || input.ModelId is null || input.Variant is null ||
            ...
            string.IsNullOrWhiteSpace(input.Name) || input.Name.Length > 120 || input.ProviderId.Length > 120 || input.ModelId.Length > 120 || input.Variant.Length > 40 ||
        ...
        if (await db.Commands.FindAsync(input.Id) is { } prior)
        {
            ...
            _ = Same(prior, service.Id, null, prior.Kind, payload);
        ...
        if (await db.ControlSessions.SingleOrDefaultAsync(x => x.ScopeKind == input.ScopeKind && x.ScopeId == input.ScopeId) is { } existing)
        {
        ...
        if (await db.ControlSessions.CountAsync() >= 64) throw new ControlException("Control session registration limit reached.");
    ...
    public Task<bool> BindControlSession(string commandId, JsonElement native, List<ModelChoice> models) => Write(async db =>
    {
        ...
        binding.NativeSessionId = nativeId; binding.State = "Ready"; binding.Detail = "Persistent control conversation is bound."; binding.Revision++;
        command.State = Delivery.Finished; command.ResultId = binding.Id; command.Detail = binding.Detail; command.UpdatedAt = Now;
        Event(db, "ControlSessionBound", runtime.Id, binding.WorkerId, command.Id, new { binding.Id, nativeId, binding.ScopeKind, binding.ScopeId });
        return true;
    });
    ...
    public Task<CoordinationRun> MigrateControlSession(string runId, MigrateControlSessionInput input) => Write(async db =>
    {
        ...
        if (!targetRuntime.DesiredConnected || targetRuntime.Transport != "Connected" || targetRuntime.Health != "Healthy" || target.Archived)
        ...
        if (binding.ScopeKind != "Workgroup" || binding.State != "Ready" || target.Stale || target.Activity != "Idle" || target.LastObservedAt is null || target.LastObservedAt < Now - 15000)
        ...
        if (await db.CoordinationRuns.AnyAsync(x => x.CoordinatorWorkerId == target.Id && x.Id != runId && x.State != "Completed" && x.State != "Stopped") ||
            ...
            throw new ControlException("Drain both control sessions before migration; existing commands and work remain independent.");
        ...
        Event(db, "CoordinationControlSessionMigrated", target.RuntimeId, target.Id, receipt.Id,
            new { run.Id, previousWorkerId, previousDecisionCommandId, controlSessionId = binding.Id, binding.NativeSessionId }, provenance: "user");
        return run;
    });
