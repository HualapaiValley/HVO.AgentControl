...
public static class RemoteWorkerCommandBuilder
{
    ...
    public const string ImageVerifyScript =
        ...
        "def cache_ok():\n  import subprocess\n  try: out=subprocess.run([\"/sbin/ldconfig\",\"-p\",\"-C\",C+\"/etc/ld.so.cache\"],capture_output=True,text=True,timeout=30).stdout\n  except Exception: return False\n  ok=(\"/lib/x86_64-linux-gnu\",\"/usr/lib/x86_64-linux-gnu\",\"/lib/aarch64-linux-gnu\",\"/usr/lib/aarch64-linux-gnu\",\"/lib64\",\"/usr/lib64\")\n  for line in out.splitlines():\n    if \"=>\" not in line: continue\n    target=line.split(\"=>\",1)[1].strip()\n    if not any(target==d+\"/\"+os.path.basename(target) for d in ok): return False\n  return True\n" +
