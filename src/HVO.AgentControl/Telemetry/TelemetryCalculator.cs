...
/// separately from absolute core usage; host metrics are never merged into either.
/// </summary>
public static class TelemetryCalculator
{
    public static RuntimeTelemetry Compute(RuntimeTelemetrySample? previous, RuntimeTelemetrySample current, TelemetryCalculatorOptions? options = null)
    {
        ...
        var windowMs = current.ObservedAt - previous.ObservedAt;
        if (windowMs <= 0)
        ...
        if (quota is null)
        {
            ...
            note = current.Platform == TelemetryParser.PlatformMacos
