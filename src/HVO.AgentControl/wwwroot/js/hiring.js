...
if (root) {
    ...
    const render = (requests) => {
        ...
        for (const request of requests) {
            ...
            const approve = document.createElement('button'); approve.type = 'button'; approve.className = 'btn'; approve.textContent = 'Approve'; approve.disabled = true; approve.title = '#217 operational acceptance must complete before approval/provisioning'; approve.setAttribute('aria-describedby', `approval-${request.id}`);
            const explanation = document.createElement('span'); explanation.id = `approval-${request.id}`; explanation.className = 'control-note'; explanation.textContent = '#217 operational acceptance must complete before approval/provisioning.';
