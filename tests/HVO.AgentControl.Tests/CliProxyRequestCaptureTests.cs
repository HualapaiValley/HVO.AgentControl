...
public sealed class CliProxyRequestCaptureTests
{
    ...
    [Theory]
    [InlineData(HttpStatusCode.OK, "configured", true)]
    [InlineData(HttpStatusCode.Unauthorized, "revoked", false)]
    [InlineData(HttpStatusCode.InternalServerError, "unavailable", false)]
        ...
        bool childStarts)
    {
        ...
        await using var catalog = await FakeCatalogEndpoint.StartAsync(DisposableKey, responseStatus);
    ...
    private static async Task<CapturedRun> RunCapturedHostAsync(string model, string variant)
    {
        ...
        var catalog = await FakeCatalogEndpoint.StartAsync(DisposableKey, HttpStatusCode.OK);
        try
        {
            var data = dataRoot.Path;
    ...
    private sealed class FakeCatalogEndpoint : IAsyncDisposable
    {
        ...
        private FakeCatalogEndpoint(TcpListener listener, string key, HttpStatusCode status)
        {
            _listener = listener;
            _key = key;
            _status = status;
        ...
        public static Task<FakeCatalogEndpoint> StartAsync(string key, HttpStatusCode status)
        {
            var listener = new TcpListener(IPAddress.Loopback, 0);
            listener.Start();
            return Task.FromResult(new FakeCatalogEndpoint(listener, key, status));
        }
        ...
        private async Task ServeAsync()
        {
            while (!_lifetime.IsCancellationRequested)
            {
                ...
                var body = status == HttpStatusCode.OK ? "{\"data\":[]}" : "{\"error\":\"denied\"}";
                var response = $"HTTP/1.1 {(int)status} {status}\r\nContent-Type: application/json\r\nContent-Length: {body.Length}\r\nConnection: close\r\n\r\n{body}";
                Assert.Equal("GET /v1/models HTTP/1.1", requestLine);
                await stream.WriteAsync(System.Text.Encoding.ASCII.GetBytes(response), _lifetime.Token);
            }
        }
    }
}
