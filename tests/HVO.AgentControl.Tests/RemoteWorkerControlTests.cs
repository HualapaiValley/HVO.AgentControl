...
public sealed class RemoteWorkerControlTests
{
    ...
    public void ExactReleasedV3MigratesWithVerifiedCreateOnceBackupAndPreservesPolicy()
    {
        ...
        using (var connection = Open(path))
        {
            ...
            foreach (var table in new[] { "hire_request_approvals", "managed_enrollment_resources", "employee_rebuilds" }) connection.Execute($"DROP TABLE {table}");
